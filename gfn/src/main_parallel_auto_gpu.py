#!/usr/bin/env python3
"""
자동 GPU 감지 기능이 포함된 main_parallel.py
외부 서버에서도 안전하게 실행 가능
"""
...
def detect_available_gpus():
    ...
    for i in range(num_gpus):
        try:
            ...
            # 간단한 텐서 생성으로 GPU 작동 확인
            test_tensor = torch.ones(1).cuda(i)
            del test_tensor
            
            available_gpus.append(i)
        except Exception as e:
            print(f"⚠️  GPU {i} is not accessible: {e}")
...
def parse_arguments():
    ...
    parser.add_argument("--auto_gpu", action="store_true", default=True,
                        help="Automatically detect and use available GPUs")
...
def train_single_problem(args_dict):
    ...
    try:
        ...
        print(f"[Process {process_id}] Warning: Failed to initialize wandb: {e}")
        run = None
    ...
    try:
        ...
        print(f"[Process {process_id}] Error in problem {prob_index}: {e}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    if args.auto_gpu and args.gpu_ids is None:
        available_gpus = detect_available_gpus()
        if available_gpus:
            args.gpu_ids = available_gpus[:min(2, len(available_gpus))]  # Use up to 2 GPUs
            print(f"✅ Auto-detected GPUs: {args.gpu_ids}")
        else:
            args.gpu_ids = []
            print("⚠️  No GPUs detected. Using CPU mode.")
    
    # Set num_processes based on available resources
    if args.num_processes is None:
        if args.gpu_ids:
            args.num_processes = min(len(args.gpu_ids), len(args.problems))
        else:
            args.num_processes = min(2, len(args.problems))  # CPU mode: use 2 processes
    ...
    for i, prob_index in enumerate(args.problems):
        if args.gpu_ids:
            ...
            print(f"  Problem {prob_index} -> CPU")
    print(f"{'='*60}\n")
