...
while step < {args.max_training_steps}:
    ...
    if not torch.equal(state, original_state) if isinstance(state, torch.Tensor) else not np.array_equal(state, original_state):
