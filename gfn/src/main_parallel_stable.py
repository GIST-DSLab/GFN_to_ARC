...
def parse_arguments():
    ...
    # Training hyperparameters - 더 안정적인 값들
...
def create_training_script(problem_id, gpu_id, args, script_path):
    """Create a training script for a single problem."""
...
print(f"🚀 Training problem {problem_id} on GPU {gpu_id}")
...
original_clip_value = 0.5  # Default in train.py
...
while step < {args.max_training_steps}:
    ...
    try:
        ...
        if step % 100 == 0:
            print(f"Step {{step}}: grad_norm={{total_grad_norm:.4f}}, lr={{optimizer.param_groups[0]['lr']:.2e}}, explore={{exploration_rate:.3f}}")
...
def main():
    ...
    print("🚀 Starting Parallel GFlowNet Training with Stable Hyperparameters")
    ...
    summary_path = os.path.join(args.output_dir, "training_summary_stable.json")
